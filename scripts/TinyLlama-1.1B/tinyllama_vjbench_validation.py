...
validation_folder = os.path.join(ROOT_PATH, "Model_patches","validation", "tinyllama_vjbench")
if not os.path.exists(validation_folder):
    os.makedirs(validation_folder)



def tinyllama_output_to_patch(output):
    # Remove special tokens and extract code
    start_index = 0
    if '<|assistant|>' in output:
        start_index = output.index('<|assistant|>') + len('<|assistant|>')
    if '</s>' in output:
...
def validate_tinyllama_vjbench(vul_id):
    ...
    if not flag:
        return
    results = tinyllama_result["data"][vul_id]
    ...
    validation_result_file_name = "{}_{}.json".format(os.path.basename(tinyllama_output_file)[:-5], vul_id )
    ...
    for i in range(len(outputs)):
        ...
        if i < existing_validation_result_length:
            continue
        generated_code = tinyllama_output_to_patch(outputs[i])
        ...
        for v in validation_result:
            ...
            if check_dupliacted == check_v:
                ...
                tinyllama_result["data"][vul_id] = results
            # write the tinyllama_result back to the json file
                with open(validation_result_file, "w") as f:
                    json.dump(tinyllama_result["data"][vul_id], f, indent=4)
                print("duplicated")
                break
        # exit()
...
if __name__ == '__main__':
    ...
    for model_name in ["TinyLlama-1.1B-Chat-v1.0"]:
        for trans in [ "rename+code_structure","structure_change_only","rename_only","original"]:
            tinyllama_output_file = os.path.join(ROOT_PATH,"Model_patches","model_output","TinyLlama-1.1B","output-TinyLlama-1.1B-Chat-v1.0-{}.json".format(trans))

            with open(tinyllama_output_file, "r") as f:
                tinyllama_result = json.load(f)
            validate_tinyllama_vjbench("Halo-1")
            exit()
            p = multiprocessing.Pool(1)
            p.map(validate_tinyllama_vjbench, bug_range_list )
