...
def get_tinyllama_input(buggy_file, rem_start, rem_end, tmp_file):
    os.chdir(JAVA_DIR)
    command([
...
def generate_tinyllama_input(output_file, trans):
    begin =  "class Main {\n"
    end = "\n}"
    tinyllama_input = {'config': 'tinyllama', 'data': {}}
    for vul_int in bug_range_list:
        vul_id = "VUL4J-{}".format(vul_int)
        if vul_int > 1000:
            vul_id = cve_int_to_name[vul_int]
        VUL_FOLDER = os.path.join(TINYLLAMA_DIR+'../../VJBench-trans', vul_id)
        ...
        tmp_code_file = os.path.join(TINYLLAMA_DIR, "tmp_code.java")
        ...
        tmp_file = TINYLLAMA_DIR + 'tmp.json'
    
        get_tinyllama_input(buggy_file_path,  buggy_line_start, buggy_line_end, tmp_file)
        ...
        json.dump(tinyllama_input, open(output_file, 'w'), indent=2)      
...
if __name__ == '__main__':
    ...
    for trans in [
        ...
        input_folder = os.path.join(TINYLLAMA_DIR,"inputs")
        if not os.path.exists(input_folder):
            os.mkdir(input_folder)   
        input_file = os.path.join(TINYLLAMA_DIR,"inputs","input-{}.json".format(trans))
        generate_tinyllama_input(input_file, trans)
